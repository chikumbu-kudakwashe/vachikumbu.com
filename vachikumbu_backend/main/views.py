...
from .serializers import (
    HighlightSerializer, SkillSerializer, AboutDataSerializer,
    CertificationSerializer, TestimonialSerializer, ContactSerializer, ReviewSerializer
)
...
class AboutDataView(generics.RetrieveUpdateAPIView):
    """Single-instance About page — always works with the first (and only) record."""
    serializer_class = AboutDataSerializer
    permission_classes = [AdminWriteOrReadOnly]

    def get_object(self):
        obj, _ = AboutData.objects.prefetch_related('highlights', 'skills').get_or_create(pk=1)
        return obj
...
class TestimonialViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        email_data = {
            'name': instance.name,
            'subject': f"New Testimonial from {instance.name}",
...
class ReviewViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        email_data = {
            'name': instance.name,
            'subject': f"New Portfolio Review from {instance.name}",
